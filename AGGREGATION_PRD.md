...
Aggregation behavior over an empty logical input, or over a projected member
with no remaining non-null values, must be explicit.
...
- `stdevp` returns `null`
- `skew` returns `null`
- `kurtosis` returns `null`
- `stringAgg` returns the empty string
- `stringAggDistinct` returns the empty string

These are data semantics, not errors.
...
- prefer generated sibling fields and generated result/input types over schema
  mutation at request time
- prefer HotChocolate filter/provider and validation hooks for parent aggregate
  predicates instead of bespoke out-of-band parsing
- prefer selection-set-aware planning so only requested operators and projected
  member fields are computed
- keep provider-backed execution and in-memory fallback behind the same public
  schema contract
...
Aggregation must define not only what is valid, but when invalid requests fail.
...
- Referencing invalid fields inside aggregate `having`, group `having`, or group
  `order` is invalid.
- Invalid parent aggregate criteria inside a parent `where` are invalid in the
  same way; they are not treated as false predicates.
- These failures must be rejected before provider execution, collection scan,
  grouping, or aggregate evaluation begins.
- `Aggregate` resolving to `null` because `having` rejected the single logical
  row is not an error.
- `Group` resolving to `[]` because no grouped rows survive `having` is not an
  error.

The exact error message text is non-normative. The required behavior is that
the request fails as a GraphQL request-validation or execution-planning error
before aggregate work begins.
