...
## HotChocolate Fit Principles

The canonical implementation model must stay inside normal HotChocolate
architecture, utilities, pipelines, and extension points.

Directive principles:

- keep the schema static at request execution time; generate public field and
  type shapes at schema-build time rather than mutating schema shape per request
- prefer type extension, generated input and object types, convention/provider
  integration, validation, field middleware, planning logic, and result
  serialization hooks over custom executor replacement
- use request validation after argument coercion for rules that depend on the
  actual argument values, such as `expand` compatibility
- reuse HotChocolate naming, scalar serialization, dependency injection, and
  normal GraphQL validation wherever those mechanisms already fit the feature
- use directives only to modify behavior of already-valid schema fields; do not
  use directives to invent new selectable output fields at request time
- use root-output directives only for response-format concerns such as export;
  they may change response serialization, but they do not change field shape
- prefer provider-backed execution where practical, but provider fallback logic
  must still preserve the canonical surface and semantics in the PRDs

These are implementation-fit rules, not alternative API suggestions. If a
proposed implementation cannot fit those principles, the implementation should
change rather than the public spec silently drifting.
...
## Cross-Cutting Error and Transport Contract

These rules apply across both feature families.

- Message text is non-normative. Semantics, failure category, and failure timing
  are normative.
- Invalid request shapes must be rejected before the underlying collection is
  scanned, expanded, grouped, aggregated, or streamed.
- When a rule can be checked during normal GraphQL validation, normal GraphQL
  validation should reject it there.
- When a rule depends on coerced argument values, it must be rejected during
  request validation or execution planning after coercion and before resolver
  work begins.
- A data outcome that is explicitly part of the public contract is not an
  error. Examples:
  - `Aggregate` eliminated by `having` resolves to `null`
  - `Group` or `FlatGroup` with no surviving rows resolves to `[]`
  - `Flat` with no emitted rows resolves to `[]`
- Unless `@export(format: CSV)` is active on a valid root export target,
  responses use the normal GraphQL JSON envelope.
- Export-validation failures still use the normal GraphQL JSON error envelope.
  They must not emit partial CSV bytes.
- Successful export responses serialize only the selected exported root field's
  rowset. They do not wrap the CSV payload in a GraphQL JSON envelope.
