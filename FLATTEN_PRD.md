...
- successful CSV export responses use `Content-Type: text/csv; charset=utf-8`
- when `fileName` is supplied, emit `Content-Disposition: attachment` with that
  filename
- the CSV payload is UTF-8 text
- the response body is the CSV payload only; it is not wrapped in a GraphQL
  JSON envelope
...
- export should stream rows where practical rather than materializing the full
  file in memory
- use `\r\n` row terminators
- serialize `null` leaf values as empty CSV fields
- object and list values are not export columns; only scalar and enum leaf
  selections may become CSV fields
...
Implementation-fit directives:

- generate flat sibling fields and flat row types at schema-build time
- use argument-coercion-aware validation for `expand` compatibility checks
- use field middleware or equivalent planning hooks for row expansion and rowset
  shaping rather than out-of-band executor replacement
- use HotChocolate result serialization or HTTP-layer hooks for CSV output,
  keeping export behavior attached to the GraphQL operation rather than a
  separate bespoke endpoint
...
## Flatten and Export Error Contract

Canonical failure rules:

- invalid `expand` path traversal is rejected before row expansion
- selecting generated flat fields that are incompatible with the requested
  `expand` is rejected before row expansion
- referencing incompatible generated fields in `where`, `by`, `having`, or
  grouped `order` is rejected before row expansion
- `@export` on nested flat fields, on non-flat fields, or on `FlatAggregate` is
  rejected before execution
- an export operation with more than one root field is rejected before
  execution
- invalid export separators and duplicate resolved CSV headers are rejected
  before execution
- successful `Flat`, `FlatAggregate`, and `FlatGroup` operations with zero
  surviving rows are data outcomes, not errors
- export validation failures return the normal GraphQL JSON error envelope and
  must not emit partial CSV bytes

The exact message text is non-normative. The normative behavior is that these
request-shape failures are detected before row expansion, grouping, aggregate
evaluation, or CSV streaming begins.
