...
- the request is rejected before row expansion begins
- this is a request-shape failure, not an empty result

## Invalid Generated-Field Selection For The Requested `expand`
...
- the request is rejected before row expansion begins
- generated fields that are not implied by the requested `expand` are invalid
  selections, even though they exist on the static flat row type

## Invalid Flat Grouping Field For The Requested `expand`
...
- the request is rejected before grouping and row expansion begin
- `by`, `having`, and grouped `order` must obey the same `expand` compatibility
  rule as selection sets and flat `where`
...
- the request is rejected before execution begins
- `@export` is valid only on one root `<field>Flat` or one root
  `<field>FlatGroup` selection
- no CSV bytes are emitted

## Invalid Export Separator Or Duplicate Headers
...
- the request is rejected before execution begins
- export validation still returns the normal GraphQL JSON error envelope
- no CSV bytes are emitted
