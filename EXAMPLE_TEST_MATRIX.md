...
- Negative cases should assert failure timing where relevant: request
  validation, execution planning, or pre-streaming export validation rather than
  only checking that "some error happened".
- Export-positive cases should ultimately assert HTTP transport behavior in an
  HTTP-level harness, not just executor-level GraphQL semantics.
...
- Transport assertions: UTF-8 payload, no GraphQL JSON envelope, and
  `Content-Disposition` when `fileName` is supplied
...
- Minimum assertions: response content type is `text/csv`, selected scalar leaf
  ...
  semantics
- Transport assertions: UTF-8 payload, no GraphQL JSON envelope, and
  deterministic column order from the leaf selection order
...
- Minimum assertions: the request is rejected before execution and does not
  enter CSV serialization or emit CSV bytes
...
- Minimum assertions: the request is rejected before execution because export
  requires exactly one exported root field and no CSV bytes are emitted
...
- Minimum assertions: the request is rejected before execution and the error
  points at the export configuration; no partial CSV is emitted

### N09. Invalid string aggregate usage

- Sources: `AGGREGATION_USAGE_EXAMPLES.md`
- Query shape: applying `stringAgg` or `stringAggDistinct` to a non-string field
- Minimum assertions: the request is rejected before aggregate execution and is
  treated as a request-shape failure rather than a data result

### N10. Invalid aggregate field inside parent filter

- Sources: `AGGREGATION_USAGE_EXAMPLES.md`
- Query shape: parent `where` using aggregate criteria that reference a missing
  projected field
- Minimum assertions: the request is rejected before parent filtering and
  aggregate execution begin; invalid aggregate criteria are not treated as false
  predicates
